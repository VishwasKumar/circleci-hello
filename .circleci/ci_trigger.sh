#!/bin/bash
set -e
# As described in https://medium.com/labs42/monorepo-with-circleci-conditional-workflows-69e65d3f1bd0
...
## Using the recent builds API, gets the most recent commit SHA for which there was a CircleCI pipeline completed
## in current branch . In case there were no builds (which is usually the case with feature branches), it attempts
## to identify the nearest parent branch and then get its most recent build commit SHA.
############################################
